...
log = logging.getLogger('stuhlgang.upgrade_database')
...
def run_script(pgconn, path_to_script):
    ...
    try:
        ...
        cursor.execute(textwrap.dedent("""
            insert into stuhlgang_schema_version
...
def maybe_run_setup_script(pgconn, setup_script):
    ...
    """
    Check if the stuhlgang_schema_version table exists.
    ...
    cursor.execute(textwrap.dedent("""
        select exists(
            ...
            where relname = 'stuhlgang_schema_version'
            and relkind = 'r'
        )
        """))
    ...
    if cursor.fetchone().exists:
        ...
        log.info("About to create stuhlgang_schema_version table.")
